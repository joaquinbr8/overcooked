...
public class LudaynaConnector implements AppConnector {
    private AndroidLibrary lib;
    private LudaynaGame game = new LudaynaGame(this);

    public LudaynaConnector(AndroidLibrary lib) {
        this.lib = lib;
    }
    ...
    public void dado(int aleatorio) {
        lib.setImageOnCell(8,8,"dado_"+aleatorio);
    }

    public void dibujarCasillaNegra(int v, int h) {
        lib.setImageOnCell(v,h,"colors_black");
    }

    public void dibujarFicha(int v, int h, String color) {
        if (color.equals("rojo")) {
            lib.setImageOnCell(v,h,"ficha_roja");
        }
        else if (color.equals("morado")){
            lib.setImageOnCell(v,h,"ficha_morada");
        }
        else if (color.equals("verde")) {
            lib.setImageOnCell(v, h, "ficha_verde");
        }
        else if (color.equals("amarillo"))  {
            lib.setImageOnCell(v,h,"ficha_amarilla");
        }
    }



    public void dibujarCasillaGris(int v, int h) {
        lib.setImageOnCell(v,h,"colors_grey");
    }

    public void anunciarTurno(String turno) {
        lib.showTemporaryMessage("Turno: " + turno);
    }

    public void llegoADestino(String color) {
        lib.showTemporaryMessage("Punto para " + color + "!!!");
    }

    public void pantallaDeVictoria(String color) {
        lib.configureScreen(1,1,0,0,false, 0);
        lib.setImageOnCell(0,0,"victory");
        lib.showTemporaryMessage("victory " + color);
    }
}
